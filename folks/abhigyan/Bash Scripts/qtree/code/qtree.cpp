...
//  printnodes();
//  nodeindex.clear();
    for(i = 0; i < nodes.size(); i++)if(nodes[i].id > b) {
            b = nodes[i].id;
        }
...
int qtree::aback() {
    ...
    for(long int k = 0; (k < max) && (l == 0); k++) {
        if (nodes[k].endnode != 'a') {
            swap = nodes[k];
            nodes[k] = nodes[0];
            nodes[0] = swap;
            l = 1;
...
int qtree::analyze() {
    ...
    while(!Divide.empty()) {
        ...
        if(error == -1) {
            std::cout << "error splitting " << i << endl;
            break;
        }
                ...
                if(nodeindex[nid] >= 0) {
                    ...
                    if(nodeindex[a] == -2) {
                        if(nodeindex[a / 5] != -2) {
                            ...
                            switch(j) {
                            case 1:
                                nodes[nodeindex[nid]].nbr[j - 1][0] = a * 5 + 3;
                                nodes[nodeindex[nid]].nbr[j - 1][1] = a * 5 + 4;
                                break;

                            case 2:
                                nodes[nodeindex[nid]].nbr[j - 1][0] = a * 5 + 1;
                            ...
                            case 4:
                                nodes[nodeindex[nid]].nbr[j - 1][0] = a * 5 + 2;
                                nodes[nodeindex[nid]].nbr[j - 1][1] = a * 5 + 4;
                                break;
                            }
                        }
...
int qtree::qjoin(int id) {
    ...
    for(int i = 1; i <= 4; i++) {
        ...
        for(int j = 0; j < 4; j++) {
            if(nodes[nodeindex[id * 5 + i]].nbr[j][0] / 5 != id) {
                NbrLess.push(nodes[nodeindex[id * 5 + i]].nbr[j][0]);
            }

            if(nodes[nodeindex[id * 5 + i]].nbr[j][1] != 0) {
                NbrLess.push(nodes[nodeindex[id * 5 + i]].nbr[j][1]);
            }
        }
...
// setting neighbors as nonexistent
    ...
    for(int j = 0; j < maxdepth; j++) {
        ...
        for(int i = 0; i < nodes.size(); i++) {
            if(nodes[i].endnode == 'b' && GetDepth(nodes[i].id) <= maxdepth) {
                Divide.push(nodes[i].id);
            }
        }
